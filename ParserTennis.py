...
m = np.array(MatchTable(read, unique, N))
#matrix = sparse.dok_matrix(MatchTable(read, unique, N))
#matrix = np.array(MatchTableWR(read, unique, N))
...
print("Modifying the match table")
matrix = np.array(MatchTable(read, unique, N))
...
while rank_score[0] < 1850 and rank_score[-1] > 100 and i < T:
    ...
    upsets = Upsets(m, rank_team, unique, N, lread)
    i+=1
...
if gradient == True:
    ...
    new_rank, upsets = ImproveRanking(matrix, rank, unique, N, d)
...
print(rank[0:10])
print("Upsets")
print(Upsets(m, rank, unique, N, lread))
...
r = UpdateScore(last, unique, rating)

print(r)

print(sum(Score(matrix, s1, unique, N)))
rank = get_rank(r, unique)
print(Upsets(m, rank, unique, N, lread))
print(rank)
"""